...
for i=1:2:2*k-1
    ...
    landmark(i:i+1,1)= pose(1:2,1) + [measure(i+1)*cos(pose(3)+measure(i));measure(i+1)*sin(pose(3)+measure(i))];
end
l_cov=0.4;
...
while ischar(tline)
    ...
    % Write your code here...
    x_pre(1:3,1)=x(1:3,1) + [d*cos(x(3,1));d*sin(x(3,1));alpha];
...
%     e_y=normrnd(0,sig_y);
    ...
    L=[cos(x(3)),-sin(x(3)),0; sin(x(3)),cos(x(3)),0;0,0,1];
    
    P_pre(1:3,1:3) = G*P(1:3,1:3)*G' + L*control_cov*L';
...
% Write your code here...
landmark_positions = [3,6,3,12,7,8,7,14,11,6,11,12];

for i=1:2:2*k-1
    plot(landmark_positions(i),landmark_positions(i+1),'*k');
end

for i=1:2:2*k
    euclidean_distance = ((x(3+i)-landmark_positions(i))^2 + (x(3+i+1)-landmark_positions(i+1))^2)^0.5;
    %disp(euclidean_distance)
    X = mvnrnd([x(3+i);x(3+i+1)],P(3+i:3+i+1,3+i:3+i+1),1000);
    Y = [landmark_positions(i),landmark_positions(i+1)];
    mahalanobis_distance = mahal(Y,X);
